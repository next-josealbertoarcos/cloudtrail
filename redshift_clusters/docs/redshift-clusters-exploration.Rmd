...
```{r}
is_interesting <- map_lgl(events, ~ .$eventName %in% c("DescribeReservedNodes", "DeleteCluster", "RestoreFromClusterSnapshot", "DeleteClusterSnapshot", "ModifyCluster", "CreateClusterSnapshot"))
...
The plot shows some clear patterns for some kind of events (like *ModifyCluter*), but it is difficult to point patterns in the relation of several different events. Let's take only the events that create and remove clusters.

We know that the only two type of existent events that are able to start or remove a cluster are:

*RestoreFromClusterSnapshot* creates a cluster from a snapshot.
*DeleteCluster* deletes a previously provisioned cluster.

Let's explore them:

```{r}
cluster_creation <- df_timeline %>% 
  filter(event %in% c("RestoreFromClusterSnapshot", "DeleteCluster"))
table(cluster_creation$event)
```

Well, something extrange here. Cluster have been destroyed 198 time, while created only 106 times... 
